...
const PizzaDetails = () => {
    ...
    return (
        <div onClick={()=> openCart == true ? dispatch(cartToggle()) : ""} className={`${openCart == true ? `brightness-[30%]` : `brightness-[100%]`} pt-[70px] h-screen bg-yellow flex flex-col gap-10 justify-center items-center`}>
            <Link onClick={()=> scrollToTop()} to={`/pizzadimama/Home`} className={`${openCart == true ? "pointer-events-none" : ""} bg-black text-white transition-all hover:brightness-[95%] top-[100px] left-[30px] fixed px-6 py-2 font-bold rounded-md`}>Close</Link>
            <div className=' relative w-[70%] h-[50%] max-md:h-auto max-sm:w-[80%] rounded-full p-5 pr-[150px] max-md:pr-5 group bg-white'>
                <div className='w-full h-full flex max-md:flex-col items-center max-md:justify-center gap-[50px]'>
                    ...
                    <div className='flex flex-col gap-6 max-md:items-center'>
                        <div className='font-extrabold font-Pencerio text-[60px] max-md:text-center'>{data[0][id].pizzaName}</div>
                        ...
                        <div className='flex flex-col gap-4 max-md:items-center'>
                            ...
                            <button className={`${openCart == true ? "pointer-events-none" : ""} relative transition-all hover:bg-[#ee5a40] font-bold bg-[#ff6146] text-white py-2 max-md:mb-10 w-[140px] text-center rounded-md`} onClick={()=> dispatch(addToCart({pizzaName: data[0][id].pizzaName, price: data[0][id].price}))}>Add To Cart
                            {
                                    cart.length > 0 ? cart.map((item, key) => {
                                        ...
                                        <div className={`size-[25px] rounded-full font-medium absolute top-[-8px] right-[-8px] bg-yellow2 text-black text-[12px] flex justify-center items-center`}>{item.quantity }</div>
                                        : null;
                                    }) : null
                            }
                            </button>
